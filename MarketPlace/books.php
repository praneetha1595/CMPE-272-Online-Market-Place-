				  ...
				  <p>
                      <?php 
    					   ...
    					   if ($conn->connect_error) 
    					   {
    					    ...
    					    $sql = "SELECT name, link, image,(review_total / review_count) as ratings FROM products WHERE store = 'bookstore'order by ratings desc limit 5";
					   $result = $conn->query($sql);
    					   $conn->close();
